...
getGiftCardModel.prototype = {
    ...
    getIt: function () {
        ...
        if (this.passWordVerify.check() && this.giftCardNum.check()) {
            var ticketCardItem = [{
                cardCode: util.trim(this.giftCardNum.value, 1),
                password: this.passWordVerify.value
            }];
...
;(function () {
    ...
    var makeqrcode = function (model) {
        ...
        setTimeout(function () {
            $("#scancode_qrcode").children("img").attr("style", "");
        }, 0);
        queryResult(model);
            ...
            cbuService.sendVerCode(function (response) {
                if (response.retCode == 0) {
                    ...
                    if (callback) callback({
                        ...
                        isError: true,
                        errorMessage: response.retMessage
                    });
                }
            }, function () {
                setStatus(self, 4);
