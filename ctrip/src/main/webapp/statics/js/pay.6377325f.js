...
var cbuService1 = {
    ...
    cardnomatch: function (successCallBack, errorCallBack, dataRequest) {
        var queryrequest = { "head": cbuService.creatHead("41001301"),
    ...
    queryUsedCard: function (successCallBack, errorCallBack, dataRequest) {
        var queryrequest = { "head": { "auth": cbuService.auth, "platform": cbuService.platform, "language": cbuService.language, "orderId": cbuService.orderId, "version": cbuService.version, "serviceCode": "41001303", "token": cbuService.token, "merchantId": cbuService.merchantId, "requestId": cbuService.requestid, "clientIp": "127.0.0.1", "guid": dataRequest.guid || OtherHelper.getGUID() },
            "chargeMode": dataRequest.chargeMode,
            "amount": dataRequest.amount,
            "currencyCode": dataRequest.currencyCode,
...
function showlimitPaySuper (ele,superEle){
    if(!$(ele).hasClass("check_disabled")) {return;}
    var top=0;
    var left=0;
    var p= $(ele);
    var limit = p.data("payLimitAmount");
    $("#"+superEle).children(".pop_txt1").html("该卡单笔限额¥"+limit+"，无法支付此订单");
//  $("#fff").css({"top":top+"px","left":left+"px"});
    var elewidth=$(ele).width();
    var Elewidth=$("#"+superEle).width();
    var Eleheight=$("#"+superEle).height()
    var _topfix=Eleheight;
    var _leftfix=Elewidth/2-elewidth/2;
    top=p.offset().top-_topfix;
    left=p.offset().left-_leftfix;
    $("#"+superEle).css({"top":top+"px","left":left+"px"});
