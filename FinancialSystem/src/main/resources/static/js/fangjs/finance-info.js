...
$(function () {
    // inquire();
    panduanBut(getParams("status"),getParams("orderId"),getParams("financeId"));
});
...
//判断出订单状态，给出按钮
function panduanBut(status,orderId,financeId) {
    var tixian="<button onclick='tuanBut("+orderId+","+financeId+")' type=\"button\" class=\"layui-btn layui-btn-lg\"\n" +
        "style=\"margin-left: 150px;margin-top: -40px\">\n" +
        "    通过\n" +
        "    </button>";
    var shixiao;
    if (status==7)
        shixiao="  <button onclick='shixiaoBut("+orderId+","+financeId+","+status+")' type=\"button\" class=\"layui-btn layui-btn-danger\" style=\"margin-left: 150px ;margin-top: -40px\"\n" +
            "                           >第二次驳回</button>";
    else
        shixiao="  <button onclick='shixiaoBut("+orderId+","+financeId+","+status+")' type=\"button\" class=\"layui-btn layui-btn-danger\" style=\"margin-left: 150px ;margin-top: -40px\"\n" +
            "                           >驳回</button>";
...
function tuanBut(orderId,financeId) {
    layer.confirm('是否通过审核？',function(){
        requestAjax("/view/alterState.htm", {orderId:orderId,status:6,financeId:financeId}, function (data) {
            if (data.result){
...
function shixiaoBut(orderId,financeId,status) {
    ...
    if (tt==''){
        ...
        layer.confirm('是否驳回该订单？',function(){
            requestAjax("/view/alterState.htm", {orderId:orderId,status:status,financeId:financeId,returnReson:tt}, function (data) {
                if (data.result){
                    //发异步，把数据提交给php
                    layer.alert("已驳回", {
                            icon: 1
