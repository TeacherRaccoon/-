...
//发送请求到后台拿数据
//分页记录数
function inquire(windId) {
    if (windId==null||windId==''){
        $("#t2").remove();
        $("#t1").remove();
    }
...
//制作分页条
function articlePage(page,windId) {
    ...
    layui.use(['layer', "laypage"], function () {
        ...
        laypage.render({
            ...
            , jump: function (obj, first) {
                //obj包含了当前分页的所有参数
                pageData(obj.curr, obj.limit,windId);
                //首次不执行
                if (!first) {
                    //do something
                }
...
//分页数据
function pageData(curr, limit,windId) {
    var createDate = $("#start").val();//订单创建时间
    var orderId = $("#orderId").val();//订单号
    var userName = $("#userName").val();//借款人姓名
    var status = $("#status").val();//订单状态
        ...
        function (data) {
            if (data.result) {
                ...
                if (data.data.length==0) {
                    ...
                    //制作分页条
                    renderPageData(data.data,windId)
                }
...
//数据渲染
function renderPageData(infoEntrylist,windId) {
    ...
    for (var i = 0; i < infoEntrylist.length; i++) {
        ...
        $tr.find("#a1").attr("onclick","xadmin.open('客户信息','wind-info.html?status="+infoEntrylist[i].status+"&orderId=' + "+infoEntrylist[i].orderId+"+'&windId='+"+windId+",$(window).width()*0.9,$(window).height() - 50)");
